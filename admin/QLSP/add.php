...
if (isset($_POST['sbm'])) {
  ...
  if (mysqli_num_rows($res) > 0) {
    while ($row = mysqli_fetch_array($res)) {
      ...
      $sql_QLSP = "UPDATE SANPHAM SET DonGia='$price_output',HinhAnh='$image1',MaDM='$category_id',TrangThai='$status1',MoTa='$description',SoLuongTon=SoLuongTon+'$input_quality',TyLeThue='$input_rate' where MaSP='$masp' and MaNCC='$mancc'";
  ...
  //-------------------------------------------------------------------------
    ...
    $sql_QLSP = "INSERT INTO sanpham(MaSP,MaNCC,TenSP,DonGia,HinhAnh,MaDM,TrangThai,MoTa,SoLuongTon,TyLeThue) VALUES ('$prd_id','$prd_supplier','$prd_name','$price_output','$image1','$category_id','$status1','$description','$input_quality',$input_rate)";
    $query_input_output = mysqli_query($conn, $sql_input_output);
    $query_QLSP = mysqli_query($conn, $sql_QLSP);
    move_uploaded_file($image_tmp, './assets/images/images_product/' . $image1);
    echo "<script>window.location.href='./index.php?url=qlsanpham&&page=1&kq=$query_QLSP'</script>";
...
<div class="container-fluid">
  ...
  <div class="card-body">
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        <label for="">Tỷ lệ thuế</label>
        <input type="numeric" name="input_rate" value="<?php 
          $sql= "SELECT TyLeThue FROM thue WHERE CURRENT_DATE() BETWEEN TuNgay AND DenNgay";
